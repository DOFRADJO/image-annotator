...
def annotate_image(image_name, label, user_id):
    ...
    return {
        "status": "success",
        "message": f"Image '{image_name}' annotée avec succès.",
        "annotation": data
    }
...
def add_class(label):
    if classes_col.find_one({"label": label}):
        return {"status": "error", "message": "Classe déjà existante"}
    classes_col.insert_one({"label": label})
    return {"status": "success", "message": f"Classe '{label}' ajoutée avec succès"}
